...
namespace Threadle.CLIconsole.Commands
{
    ...
    public class RandomWalkerFirstPassageTime : ICommand
    {
        ...
        /// </summary>
        public string Syntax => "[var:network] = rwfpt(network = [var:network], attrname = [str], maxsteps = [uint], *layernames = [semicolon-separated], *walkfactor = [float(default=1.0)], *minpairobs = [int(default=10)], *balanced = ['false'(default),'true'], *weighted = ['false'(default),'true'], *backtrack = ['false'(default),'true'], *savesteps = ['false'(default),'true'])";

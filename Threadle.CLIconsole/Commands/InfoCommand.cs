...
namespace Threadle.CLIconsole.Commands
{
    internal class InfoCommand : ICommand
    {
        ...
        public void Execute(Command command, CommandContext context)
        {
            ...
            //StructureMetadata infoMetadata = structure.Info;
            //if (outputFormat.Equals("console"))
            //    ConsoleOutput.WriteLine(infoMetadata.ToString());
            //else
            //{
            //    if (structure.Info is NetworkMetadata networkDto)
            //        ConsoleOutput.WriteJson(networkDto);
            //    //ConsoleOutput.WriteLine(JsonSerializer.Serialize(networkDto, new JsonSerializerOptions { WriteIndented = true }));
            //    else if (structure.Info is NodesetMetadata nodesetDto)
            //        //ConsoleOutput.WriteJson(nodesetDto);
            //        ConsoleOutput.WriteLine(JsonSerializer.Serialize(nodesetDto, new JsonSerializerOptions { WriteIndented = true }));
            //}
        }
    }
}

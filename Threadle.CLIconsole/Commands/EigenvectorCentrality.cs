...
namespace Threadle.CLIconsole.Commands
{
    ...
    public class EigenvectorCentrality : ICommand
    {
        ...
        /// </summary>
        public string Syntax => "eigenvectorcentrality(network=[var:network], *layernames=[semicolon-separated], *attrname=[str], *traversal=['out'(default),'both','in'], *maxiterations=[int], *tolerance=[double])";

        /// <summary>
        /// Gets a human-readable description of what the command does.
        /// </summary>
        public string Description => "Calculates eigenvector centrality via power iteration and stores the result as a node attribute. Scores are L2-normalized. For 2-mode layers uses efficient per-hyperedge summation. Traversal for directed network is out-bound by default. Iterates until convergence within tolerance or maxiterations is reached. See Bonacich (1987).";

...
# Statement of need
Large-scale administrative register data – such as the Swedish population registers for research – are extremely high-volume and structurally complex, containing multiple relational layers (kinship, residence, employment, education etc.), many of which are two-mode (affiliation) structures. Many research applications involving multilayer random-walker models, simulation-based methods, and repeated sampling of ego networks across layers require very fast retrieval of neighboring alters and node attributes, while keeping a memory footprint small enough to allow population-scale networks, possibly for multiple years, to be simultaneously loaded into RAM.

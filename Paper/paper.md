...
igraph and networkx provide broad graph-analytic toolsets but rely on metadata-based representations of multilayer structures and often require projection for handling bipartite data, substantially increasing memory use. Their reliance on high-level language containers for attributes further contributes to large footprints.

Threadle instead introduces a native multilayer/multimode model, hyperedge-based bipartite storage, and compact attribute handling, targeting use cases where memory constraints or structural complexity exceed the capabilities of general-purpose libraries. Analytical algorithms are expected to be implemented externally, with Threadle supplying efficient access to network structure and attributes.
